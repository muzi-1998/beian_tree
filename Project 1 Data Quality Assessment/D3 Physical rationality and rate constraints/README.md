# D3 v2.8.0: Physical Rationality and Persistent-Rate Constraints
...
The proposed `0.45/0.35/0.20` rebalance remains sensitivity-only because it caused a large ORP3 soft-envelope reclassification without external labels. v2.7 retains the frozen outer weights. The historical v2.3 comparison is reconstructed from all hard point violations, matching the original `Q_rate` contract rather than the later persistent-rate evidence.
...
- Primary calibration admission uses observed, non-imputed DO, valid temperature, registered instrument range and at least 30 raw minutes per hour. No D1/D2 score is used to fit the primary envelope. The estimator and temporal splits are unchanged.
- Frozen neutral position coefficients are `0.3463141470`, `0.3715629157`, and `0.6814501118`. The original screened coefficients `0.2789373305`, `0.3100300303`, and `0.5065765789` remain sensitivity-only, not a fault-truth clean reference. Position 3 remains diagnostic-only regardless of its lower warning burden under the wider envelope.
- The already examined validation and terminal periods are revised retrospective out-of-sample evaluations, not a new blind test. See `reference_comparison` for both routes evaluated on identical neutral support. Warning frequency is not a false-positive rate without independent event labels.
- Legacy `high_quality_*` columns in the temperature workbook/parquet now mean neutral reference eligibility and are retained only for schema compatibility. The revision comparison workbook uses explicit neutral-eligibility names.
- Leave-one-line-out transfer is reported separately and is never used to tune the pooled position template.
- The combined soft-violation rate is a union probability on one explicit determinable-minute set: `temperature valid OR low-side violation`. A minute with missing temperature and no low-side violation remains unknown, not an implicit high-side pass; a confirmed low-side violation remains evaluable even when temperature is absent.
- Bootstrap lower, point, and upper alpha scenarios are propagated to minute and 2 h warning burdens as supplementary uncertainty analysis. The point estimate remains the frozen production coefficient.
...
figures/             Python scripts for eleven publication figures
...
Validation outputs additionally include `D3_DO4_zero_equivalence_views.parquet`, `D3_weight_contract_sensitivity.xlsx`, `D3_operational_envelope_diagnostics.xlsx`, `D3_temperature_conditioned_DO_upper.xlsx`, and `D3_temperature_conditioned_DO_upper.parquet`. The temperature workbook contains source QA, Benson-Krause reference checks, frozen-registry reproduction, day-block confidence intervals, minute/2 h phase validation, cross-line transfer, alpha-interval propagation, fixed-8-mg/L comparison, sensitivity, and exclusions. The operational-envelope workbook contains DO4 paired-line and ORP position-specific effect estimates with calendar-day block intervals. The Parquet file contains the complete minute-level source data for figure reproduction. D1/D2 screening is retained only as a historical sensitivity reference; it is not consumed by primary calibration, validation eligibility or production scoring. The legacy `high_quality_*` column names denote neutral raw eligibility in v2.8 and remain only for schema compatibility.
