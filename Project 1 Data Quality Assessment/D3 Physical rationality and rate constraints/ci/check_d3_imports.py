"""Static checks for D3 dimension independence and diagnostic boundaries."""
...
SCANNED = (ROOT / "src" / "d3_physical", ROOT / "src" / "pipeline")
...
def main() -> int:
    ...
    mapping = yaml.safe_load((ROOT / "configs" / "d3_mapping.yaml").read_text(encoding="utf-8"))
    weights = mapping["aggregation"]["weights"]
    if "Q_boundary" in weights:
        violations.append((Path("configs/d3_mapping.yaml"), 0, "Q_boundary must remain diagnostic-only"))
    if abs(sum(weights.values()) - 1.0) > 1e-12:
        violations.append((Path("configs/d3_mapping.yaml"), 0, "aggregation weights must sum to 1"))

    if violations:
        print("D3 contract check failed:")
        for path, line_no, message in violations:
            print(f"  {path}:{line_no}: {message}")
        return 1
    print(f"D3 contract check passed: {len(files)} source files scanned")
    return 0
