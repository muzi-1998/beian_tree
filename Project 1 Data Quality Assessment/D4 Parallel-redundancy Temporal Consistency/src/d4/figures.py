...
def figure_m1(cfg: D4Config, main: pd.DataFrame, output_dir: Path) -> None:
    ...
    save_figure(fig, output_dir / "FigD4_1_paired_residual_consistency")
...
def figure_m2(main: pd.DataFrame, profile: pd.DataFrame, output_dir: Path) -> None:
    ...
    for values, color, label in zip((means * weights).T, colors, ["Distribution", "Trend", "Variability", "Change point"]):
        ...
        bottom += values
    axes[0].set_ylabel("Weighted contribution to D4 base")
    ...
    data = [main.loc[(main["pair_id"] == pair) & main["usable_for_D4"], "D4_total"].dropna() for pair in pairs]
    ...
    axes[1].set_ylabel("D4 score (1-5)")
    ...
    save_figure(fig, output_dir / "FigD4_2_subscore_contribution")
...
def figure_m3(raw: pd.DataFrame, output_dir: Path) -> None:
    ...
    save_figure(fig, output_dir / "FigD4_3_trend_slope_scatter")


def figure_d1(main: pd.DataFrame, output_dir: Path) -> None:
    weekly = (main.set_index("timestamp").groupby("pair_id")["D4_raw"]
    ...
    cbar.set_label("Mean D4 raw score")
    fig.text(0.10, 0.965, "Independent pair-consistency evidence",
             ha="left", va="bottom", fontsize=6)
    _boxed(ax)
    fig.subplots_adjust(left=0.10, right=0.94, top=0.91, bottom=0.27)
    save_figure(fig, output_dir / "FigD4_4_raw_score_heatmap")
...
def figure_d2(main: pd.DataFrame, output_dir: Path) -> None:
    ...
    mode = weekly.groupby(["pair_id", "week"])["raw_status_label"].agg(
        lambda x: x.mode().iloc[0]
    ).unstack()
    coverage = weekly.groupby(["pair_id", "week"])["usable_for_D4"].mean().unstack()
    ...
    save_figure(fig, output_dir / "FigD4_5_status_evaluability")


def figure_d3(main: pd.DataFrame, output_dir: Path) -> None:
    valid = main.dropna(subset=["D4_raw", "D4_forDQR_provisional"]).copy()
    fig, axes = plt.subplots(1, 2, figsize=(183 / 25.4, 77 / 25.4), gridspec_kw={"wspace": 0.34})
    hb = axes[0].hexbin(valid["D4_raw"], valid["D4_forDQR_provisional"],
                        gridsize=42, mincnt=1, cmap="Blues", extent=(1, 5, 1, 5))
    axes[0].plot([1, 5], [1, 5], color=PALETTE["gray"], lw=0.75, ls="--")
    axes[0].set(xlim=(1, 5), ylim=(1, 5), xlabel="D4 raw score",
                ylabel="Provisional D4-for-DQR score")
    ...
    axes[1].text(0.98, 0.96, "D1 context only; D4 raw is protected",
    ...
    save_figure(fig, output_dir / "FigD4_6_context_independence")
...
def figure_v1(benchmark_path: Path, output_dir: Path) -> None:
    ...
    save_figure(fig, output_dir / "FigD4_7_validation_roc_pr")
...
def figure_v2(benchmark_path: Path, output_dir: Path) -> None:
    ...
    save_figure(fig, output_dir / "FigD4_8_validation_ablation")
...
def make_all_figures(cfg: D4Config, data_dir: Path, output_dir: Path) -> None:
    ...
    profile = pd.read_excel(data_dir / "D4_pair_profile_summary.xlsx", sheet_name="pair_profile")
    ...
    figure_v1(data_dir / "D4_benchmark_results.xlsx", output_dir)
    figure_v2(data_dir / "D4_benchmark_results.xlsx", output_dir)
