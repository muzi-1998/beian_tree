...
class TrackIsolationGuard:
    ...
    def validate_input_schema(self, columns: Iterable[str]) -> None:
        ...
        if offenders:
            raise TrackIsolationError(f"Forbidden upstream fields in d5_local: {offenders}")

    def validate_write_target(self, target: Path, local_root: Path) -> None:
        resolved_target = target.resolve()
        resolved_root = local_root.resolve()
        if resolved_target != resolved_root and resolved_root not in resolved_target.parents:
            raise TrackIsolationError(f"d5_local cannot write outside {resolved_root}")

    def attest_manifest(self, manifest: dict[str, object]) -> None:
        if manifest.get("track_id") != "d5_local":
            raise TrackIsolationError("Local manifest must use track_id=d5_local")
