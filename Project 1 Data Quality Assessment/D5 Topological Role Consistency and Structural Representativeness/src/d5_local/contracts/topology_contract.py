...
class TopologyRegistry:
    ...
    def _validate_frames(nodes: pd.DataFrame, edges: pd.DataFrame, pairs: pd.DataFrame) -> None:
        if len(nodes) != 14 or nodes["sensor_id"].duplicated().any():
            raise ValueError("D5 topology requires exactly 14 unique DO/ORP nodes")
        ...
        if len(pairs) != 7 or pairs["pair_id"].duplicated().any():
            raise ValueError("D5 topology requires seven unique D4 peer pairs")
        if set(pairs["sensor_a"]) | set(pairs["sensor_b"]) != node_ids:
            raise ValueError("Seven peer pairs must cover all D5 nodes exactly once")
    ...
    def _validate_research_evidence(
        ...
        inventory_reconciled = bool(
            int(reconciliation.get("d5_do_node_count", -1)) == expected_do
            and int(reconciliation.get("d5_orp_node_count", -1)) == expected_orp
