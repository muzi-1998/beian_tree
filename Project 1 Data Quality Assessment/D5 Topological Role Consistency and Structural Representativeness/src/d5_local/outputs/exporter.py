...
class D5OutputExporter:
    ...
    def write_templates(
        ...
        bundle = self.output_root / "D5_spatial_templates.template_bundle.json"
        bundle.write_text(
            json.dumps(
    ...
    def write_topology(
        ...
        yaml_target = self.output_root / "D5_topology_registry.yaml"
        schema_target = self.output_root / "D5_topology_registry.schema.json"
        evidence_target = self.output_root / "D5_topology_evidence.yaml"
        shutil.copy2(source_yaml, yaml_target)
        shutil.copy2(schema_path, schema_target)
        shutil.copy2(evidence_path, evidence_target)
        json_target = self.output_root / "D5_topology_registry.json"
    ...
    def copy_interface_schemas(
        ...
        targets = [
            self.output_root / "D5_report_interface.schema.json",
            self.output_root / "D5_gate_interface.schema.json",
        ]
