    ...
    *,
    ...
    for pair in topology.twin_pairs.itertuples(index=False):
        ...
        for timestamp in timestamps:
            ...
            abnormal_count = int((zone_frame["D5_raw"] < 3.0).sum())
            d5_evaluable = bool(
                target_score["evaluation_status"] == "evaluable"
                and reference_score["evaluation_status"] == "evaluable"
                and np.isfinite(target_score["D5_report_score"])
                and np.isfinite(reference_score["D5_report_score"])
                and topology.research_topology_confirmed
            )
            if not np.isfinite(target_score["D5_raw"]) or not np.isfinite(reference_score["D5_raw"]):
            ...
            elif target_score["D5_raw"] >= 3.5 and reference_score["D5_raw"] >= 3.5:
                label = "spatially_consistent"
                strength = min(1.0, min(target_score["D5_raw"], reference_score["D5_raw"]) / 5.0)
            else:
                label = "inconclusive"
            ...
            rows.append(
                {
                    ...
                    "target_D5_report": target_score["D5_report_score"],
                    "reference_D5_report": reference_score["D5_report_score"],
                    "neighbor_abnormal_count": abnormal_count,
                    "evidence_count": len(zone_frame),
                    "direction": "unknown",
                    "d5_evaluable": d5_evaluable,
                    "support_level": support_level,
                    "limited_support": bool(
                        target_score["limited_support"] or reference_score["limited_support"]
                    ),
                    "d5_score_ready": d5_evaluable,
                    "d5_action_candidate": bool(
                        d5_evaluable
                        and support_level == "L3"
