...
from d5_local.events import build_events, build_zone_consensus
...
from d5_local.scoring import ApplicabilityGate, ScoreMapper, UncertaintyEngine, aggregate_scores
from d5_local.templates import ORPDegradationPolicy, SpatialTemplateBuilder
from d5_local.topology import TopologyDriftMonitor
...
class D5Pipeline:
    """Batch implementation of the D5 v2.3 Local Track contract."""

    def __init__(self, *, max_input_rows: int | None = None) -> None:
        self.paths = resolve_paths()
        self.max_input_rows = max_input_rows
        self.common_root = D5_ROOT / "configs" / "common"
        self.local_root = D5_ROOT / "configs" / "local"
        self.config = load_yaml(self.local_root / "d5_local.yaml")
    ...
    def run(self) -> D5RunResult:
        started = time.perf_counter()
        run_id = pd.Timestamp.utcnow().strftime("D5-LOCAL-%Y%m%dT%H%M%SZ")
    ...
    def _build_regime_state(
        ...
        for sensor in self.topology.node_ids():
            ...
            state["template_id_used"] = state["active_regime_id"].map(
                lambda regime: f"D5-{sensor}-R{int(regime)}"
            )
    ...
    def _build_main_scores(
        ...
        output["D5_base"] = d5_base
        output["D5_raw"] = d5_raw
        ...
        ordered = [
            ...
            "regime_state", "Q_profile", "Q_gradient", "Q_rank", "Q_rep", "D5_base",
            "D5_raw", "D5_report_provisional", "D5_report", "D5_total",
            "D5_report_score",
    ...
    def _build_reference_library(
        ...
        daily = reference.groupby(["date", "sensor_id", "active_regime_id"], as_index=False).agg(
            ...
            leverage=("D5_raw", lambda values: float((5.0 - values.median()) / 4.0)),
            template_id=("template_id_used", "last"),
        )
        daily["reference_id"] = daily.apply(
            lambda row: f"D5-REF-{row.sensor_id}-{row.date:%Y%m%d}", axis=1
        )
        daily["track_id"] = "d5_local"
    ...
    def _sensor_summary(
        ...
        summary = main.groupby(["sensor_id", "analyte", "line_id", "zone_id"], as_index=False).agg(
            n_windows=("D5_raw", "size"),
            D5_raw_mean=("D5_raw", "mean"),
            D5_raw_p05=("D5_raw", lambda values: values.quantile(0.05)),
            D5_raw_p50=("D5_raw", "median"),
            low_score_rate=("D5_raw", lambda values: values.lt(3.0).mean()),
    ...
    def _multiscale(self, main: pd.DataFrame) -> dict[str, pd.DataFrame]:
        ...
        daily = frame.groupby(["day", "sensor_id", "analyte", "line_id"], as_index=False).agg(
            daily_gate_p05=("D5_total", lambda values: values.quantile(0.05)),
            daily_report_p25=("D5_raw", lambda values: values.quantile(0.25)),
            coverage=("window_coverage", "mean"),
        )
        weekly = frame.groupby(["week", "sensor_id", "analyte", "line_id"], as_index=False).agg(
            weekly_report_p25=("D5_raw", lambda values: values.quantile(0.25)),
            weekly_report_median=("D5_raw", "median"),
            coverage=("window_coverage", "mean"),
        )
        pool = frame.groupby(["timestamp", "analyte", "line_id"], as_index=False).agg(
            report_p25=("D5_raw", lambda values: values.quantile(0.25)),
            gate_p05=("D5_total", lambda values: values.quantile(0.05)),
        )
    ...
    def _case_studies(
        ...
        for sensor in selected:
            sensor_frame = main[(main["sensor_id"] == sensor) & main["D5_raw"].notna()]
            if sensor_frame.empty:
                continue
            center = sensor_frame.nsmallest(1, "D5_raw").iloc[0]
            ...
            index_rows.append(
                {
                    ...
                    "case_type": "observed_low_D5_raw_review",
    ...
    def _audit_frames(
        ...
        run = pd.DataFrame(
            [{
                ...
                "d5_version": self.config["version"],
            }]
        )
        ...
        d4_handshake = pd.DataFrame(
            [{
                ...
                "finalized_rows": 0,
                "status": "pending_D4_handshake_production_approval_or_support",
            }]
        )
        ...
        return {
            ...
            "schema_qa": schema_qa,
            "d4_handshake": d4_handshake,
            "publication_qa": publication_qa,
        }

    def _export_all(self, **artifacts: Any) -> None:
        self.exporter.write_dual("D5_main_scores_hourly", artifacts["main"], "main_scores")
        self.exporter.write_dual("D5_spatial_evidence", artifacts["spatial_evidence"], "evidence")
        self.exporter.write_dual("D5_regime_state", artifacts["regime_state"], "regime_state")
        self.exporter.write_dual("D5_support_assessment", artifacts["support"], "support")
        self.exporter.write_dual("D5_sensor_influence", artifacts["influence"], "influence")
        self.exporter.write_dual("D5_zone_consensus", artifacts["consensus"], "zone_consensus")
        ...
        self.exporter.write_dual("D5_event_windows", artifacts["events"], "events")
        self.exporter.write_dual("D5_reference_window_library", artifacts["reference_library"], "reference")
        self.exporter.write_dual("D5_topology_drift_alerts", artifacts["drift"], "drift_alerts")
        ...
        self.exporter.copy_interface_schemas(
            self.common_root / "d5_report_interface.schema.json",
            self.common_root / "d5_gate_interface.schema.json",
        )
        self.exporter.write_workbook(
            "D5_sensor_profile_summary", {"sensor_profile": artifacts["sensor_summary"]}
        )
        self.exporter.write_workbook(
            "D5_edge_profile_summary", {"edge_profile": artifacts["edge_summary"]}
        )
        self.exporter.write_workbook("D5_multiscale_aggregates", artifacts["multiscale"])
        self.exporter.write_workbook("D5_case_study_exports", artifacts["case_studies"])
        self.exporter.write_workbook("D5_audit_log", artifacts["audit"])
    ...
    def _write_manifest(
        ...
        build_manifest(
            ...
            scientific_boundaries=[
                "D5_raw measures spatial role consistency and structural representativeness, not sensor health, temporal availability, physical rate plausibility, or D4 temporal synchronization.",
                ...
                "L2/L3 evidence can populate the retrospective D5 report interface; only family-L3 templates that pass node validation may enter the gate interface.",
    ...
    def _schema_checks(
        ...
        return [
            {"artifact": "D5_main_scores_hourly", "check": "primary_key_unique",
             "passed": not main.duplicated(["timestamp", "sensor_id"]).any()},
            {"artifact": "D5_main_scores_hourly", "check": "score_bounds",
             "passed": bool(main[["Q_profile", "Q_gradient", "Q_rank", "Q_rep", "D5_raw"]].stack().between(1, 5).all())},
            {"artifact": "D5_main_scores_hourly", "check": "local_track_isolation",
             "passed": bool((main["track_id"] == "d5_local").all() and ~main["upstream_score_consumed"].any())},
            {"artifact": "D5_main_scores_hourly", "check": "scientific_scores_follow_eligibility",
             "passed": bool(
                 main.loc[main["score_eligible"], "D5_total"].notna().all()
                 and main.loc[~main["score_eligible"], "D5_total"].isna().all()
                 and main["D5_report_score"].equals(main["D5_total"])
              )},
            {"artifact": "D5_regime_state", "check": "primary_key_unique",
             "passed": not regime.duplicated(["timestamp", "sensor_id"]).any()},
            {"artifact": "D5_support_assessment", "check": "ORP_supported_diagonal_alpha1",
            ...
            {"artifact": "D5_support_assessment", "check": "ORP_L0_disabled",
