...
class D5SensitivityPipeline:
    ...
    def run(self) -> dict[str, Any]:
        ...
        shadow.loc[~np.isfinite(matrix).all(axis=1), "shadow_D5"] = np.nan
        shadow["shadow_status"] = np.where(
            shadow["shadow_D5"].notna(), "shadow_evaluable", "shadow_not_evaluable"
        )
        shadow["shadow_template_id"] = shadow["template_id_used"].astype(str) + "-SENS"
        shadow["consumed_sources"] = "D1_total_hourly;D2_total_wide;D3_total"
        shadow["track_id"] = "sensitivity"
        keep = [
            "timestamp", "sensor_id", "analyte", "active_regime_id", *q, "shadow_D5",
            "shadow_status", "shadow_template_id", "upstream_filter_pass", "D1_score",
            "D2_score", "D3_score", "consumed_sources", "track_id", "topology_hash",
            "template_version",
        ]
        shadow = shadow[keep]
        invariance = self._invariance(local, shadow, influence)
        template_shift = pd.DataFrame(shift_rows)
        self._write_dual("D5_shadow_scores", shadow)
        ...
        self._write_workbook(
            "D5_shadow_template_shift", {"quantile_shift": template_shift}
        )
        ...
        dependency_paths = {
            ...
            "D3_scores": self.paths.d3_scores,
            "D5_local_evidence": self.paths.local_output_root / "D5_spatial_evidence.parquet",
        }
        workspace_root = D5_ROOT.parent.resolve()
        ...
        manifest = {
            ...
            "consumed_sources": [
                ...
                project_relative(self.paths.d3_scores),
                project_relative(
                    self.paths.local_output_root / "D5_spatial_evidence.parquet"
                ),
            ],
        ...
        (self.output_root / "D5_sensitivity_manifest.json").write_text(
    ...
    def _load_upstream(self) -> pd.DataFrame:
        ...
        d3 = pd.read_excel(self.paths.d3_scores)
        d3["timestamp"] = pd.to_datetime(d3["ts"])
        d3 = d3[["timestamp", "sensor_id", "D3_total"]].rename(
            columns={"D3_total": "D3_score"}
        )
        d3 = d3.sort_values("timestamp").drop_duplicates(
            ["timestamp", "sensor_id"], keep="last"
        )
        output = d1.merge(d2, on=["timestamp", "sensor_id"], how="outer")
        return output.merge(d3, on=["timestamp", "sensor_id"], how="outer")

    def _invariance(
        self, local: pd.DataFrame, shadow: pd.DataFrame, influence: pd.DataFrame
    ) -> pd.DataFrame:
        merged = local[["timestamp", "sensor_id", "D5_raw"]].merge(
            shadow[["timestamp", "sensor_id", "shadow_D5", "shadow_Q_rep"]],
            on=["timestamp", "sensor_id"],
            how="inner",
        )
        valid = merged.dropna(subset=["D5_raw", "shadow_D5"])
        ie = float((valid["D5_raw"] - valid["shadow_D5"]).abs().mean() / 4.0)
        local_event = set(map(tuple, valid.loc[valid["D5_raw"] < 3.0, ["timestamp", "sensor_id"]].to_numpy()))
        shadow_event = set(map(tuple, valid.loc[valid["shadow_D5"] < 3.0, ["timestamp", "sensor_id"]].to_numpy()))
        ...
        far_delta = float((valid["shadow_D5"] < 3.0).mean() - (valid["D5_raw"] < 3.0).mean())
        rows = [
