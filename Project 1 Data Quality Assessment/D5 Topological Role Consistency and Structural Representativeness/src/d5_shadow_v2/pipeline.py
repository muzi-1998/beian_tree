...
class D5GraphShadowPipeline:
    ...
    def __init__(self) -> None:
        ...
        self.sensors = load_yaml(D5_ROOT / "configs" / "common" / "sensors.yaml")["nodes"]
    ...
    def run(self) -> dict[str, object]:
                ...
                rows.append(
                    {
                        ...
                        "candidate_graph_id": f"D5-GRAPH-{left}-{right}",
                        "source": left,
                        "target": right,
                        "analyte": analyte,
        ...
        output.to_parquet(
            self.output_root / "D5_graph_structure_learning_shadow.parquet", index=False
        )
        with pd.ExcelWriter(
            self.output_root / "D5_graph_structure_learning_shadow.xlsx", engine="openpyxl"
        ) as writer:
        ...
        (self.output_root / "D5_shadow_v2_manifest.json").write_text(
