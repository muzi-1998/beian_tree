...
events = read("D4_physical_events.xlsx")
profile = read("D4_sensor_summary.xlsx").sort_values("mean_D4")
...
ax = axes[0, 0]
ax.hlines(y, profile.min_D4, profile.mean_D4, color=COLORS["light_gray"], linewidth=2.2)
ax.scatter(profile.min_D4, y, s=16, color=COLORS["red"], label="Minimum", zorder=3)
ax.scatter(profile.mean_D4, y, s=19, color=COLORS["blue"], label="Mean", zorder=3)
ax.set_yticks(y, [short_sensor(s) for s in order])
ax.set(xlabel="D4 score", xlim=(1, 5.05))
...
categories = pd.cut(evaluated.D4_total, [0, 1.6, 2.5, 3.5, 5.01], labels=["Invalid", "Review", "Report", "Train"])
