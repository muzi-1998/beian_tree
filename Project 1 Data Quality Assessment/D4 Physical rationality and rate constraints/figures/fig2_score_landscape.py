"""Figure 2: D4 score landscape across sensors and time."""
...
for group, color in [("DO", COLORS["blue"]), ("ORP", COLORS["orange"])]:
    ax.hist(scores.loc[scores.sensor_type == group, "D4_total"], bins=bins, density=True,
            histtype="step", linewidth=1.2, color=color, label=group)
ax.set(xlabel="D4 score", ylabel="Density", xlim=(1, 5))
...
ax = axes[0, 1]
pivot = scores.pivot_table(index="sensor_id", columns="month", values="D4_total", aggfunc="median")
...
ax.set_title("Monthly median D4")
style_axis(ax, full_frame=True)
fig.colorbar(image, ax=ax, fraction=0.045, pad=0.03, label="D4")
panel_label(ax, "(b)")

ax = axes[1, 0]
sample = scores.sample(min(9000, len(scores)), random_state=22)
value_score = 0.5 * sample.Q_value_hard + 0.2 * sample.Q_value_soft
scatter = ax.scatter(value_score, 0.3 * sample.Q_rate, c=sample.D4_total, cmap="viridis",
...
style_axis(ax, minor=True)
fig.colorbar(scatter, ax=ax, fraction=0.045, pad=0.03, label="D4")
panel_label(ax, "(c)")

ax = axes[1, 1]
order = list(scores.groupby("sensor_id").D4_total.mean().sort_values().index)
