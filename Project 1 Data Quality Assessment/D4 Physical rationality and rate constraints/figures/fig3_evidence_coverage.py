...
profile = read("D4_sensor_summary.xlsx")
...
gate = scores.groupby("coverage_bin", observed=False).agg(
    evaluated=("D4_total", lambda x: x.notna().mean()), n=("sensor_id", "size")
...
daily = scores.assign(day=scores.ts.dt.floor("D")).groupby("day").agg(
    evaluated=("D4_total", lambda x: x.notna().mean()), observed=("observed_fraction", "mean")
)
