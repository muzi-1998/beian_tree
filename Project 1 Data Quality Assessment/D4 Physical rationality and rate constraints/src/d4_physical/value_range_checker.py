...
class ValueRangeChecker:
    ...
    def check(self, x: np.ndarray, sensor: str, sensor_type: str) -> ValueEvidence:
        ...
        soft_low, soft_high = self.thresholds.soft_bounds(sensor_type)
        valid = np.isfinite(x)
        clean = x[valid]
        if not len(clean):
            return ValueEvidence(
                sensor, sensor_type, hard_low, hard_high, soft_low, soft_high,
                0, 0, 0, np.nan, np.nan, np.nan, False, 0,
            )

        hard = (clean < hard_low) | (clean > hard_high)
        soft = (clean < soft_low) | (clean > soft_high)
