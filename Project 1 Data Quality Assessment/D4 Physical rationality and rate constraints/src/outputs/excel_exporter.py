"""Export the D4 v2.2 evidence, diagnostics, scores, and audit tables."""
...
def export_all(
    ...
    paths = {
        "scores": outdir / "D4_window_scores.xlsx",
        "value": outdir / "D4_value_evidence.xlsx",
        "rate": outdir / "D4_rate_evidence.xlsx",
        "boundary": outdir / "D4_boundary_diagnostics.xlsx",
        "thresholds": outdir / "D4_threshold_library.xlsx",
        "events": outdir / "D4_physical_events.xlsx",
        "mapping": outdir / "D4_mapping_params.xlsx",
        "profile": outdir / "D4_sensor_summary.xlsx",
    }
    ...
    with pd.ExcelWriter(paths["thresholds"]) as writer:
        ...
        thresholds_df[thresholds_df["included_in_D4_score"]].to_excel(
...
def build_profile_summary(results: dict) -> pd.DataFrame:
    ...
    for sensor, group in scores.groupby("sensor_id"):
        ...
        rows.append({
            ...
            "mean_D4": float(evaluated["D4_total"].mean()),
            "median_D4": float(evaluated["D4_total"].median()),
            "min_D4": float(evaluated["D4_total"].min()),
