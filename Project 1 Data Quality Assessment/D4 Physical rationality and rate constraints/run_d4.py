"""Run the independent D4 v2.2 physical-plausibility pipeline."""
...
from src.pipeline.d4_pipeline import D4Pipeline
...
def main(
    ...
    paths_cfg = _load_yaml(configs_dir / "d4_paths.yaml")
    sensors_cfg = _load_yaml(configs_dir / "d4_sensors.yaml")
    physical_bounds_cfg = _load_yaml(configs_dir / "d4_physical_bounds.yaml")
    rate_limits_cfg = _load_yaml(configs_dir / "d4_rate_limits.yaml")
    mapping_cfg = _load_yaml(configs_dir / "d4_mapping.yaml")
    rules_cfg = _load_yaml(configs_dir / "d4_rules.yaml")
    dag_cfg = _load_yaml(configs_dir / "d4_dag.yaml")
    ...
    if forbidden != {"D1", "D2"}:
        raise ValueError("D4 independence contract must explicitly forbid D1 and D2 score inputs")

    source_meta = source_fingerprints(paths_cfg, ROOT)
    config_hash = _sha256_files(list(configs_dir.glob("*.yaml")))
    code_hash = _sha256_files(list((ROOT / "src").rglob("*.py")) + [ROOT / "run_d4.py"])
    created_at = datetime.now(timezone.utc)
    run_id = f"RUN_D4_{D4_VERSION}_{created_at.strftime('%Y%m%dT%H%M%SZ')}_{config_hash[:8]}"
    ...
    print("[D] Scoring D4 independently of D1/D2 and D6 regime labels...")
    configs = {
    ...
    manifest = {
        ...
        "mean_D4_evaluated": float(evaluated["D4_total"].mean()),
        "versions": {
