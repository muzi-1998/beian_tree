...
ax = fig.add_subplot(gs[1, 2])
v1_evts = pd.read_excel("/mnt/project/D1_event_windows.xlsx", "fault_summary")
# Filter to scored channels only
v1_evts = v1_evts[v1_evts["sensor_id"].isin(SCORED)]
v1_dom = v1_evts.groupby("dominant_fault")["n_events"].sum().to_dict()
# v1.1 dominant fault per event
...
v1_counts = [v1_dom.get(f, 0) for f in faults]
v11_counts = [v11_dom.get(f"Q_{f}", 0) for f in faults]
xs = np.arange(len(faults)); bw = 0.4
ax.bar(xs - bw/2, v1_counts, bw, color=C["gray"], alpha=0.85,
        label=f"V1 (DO/ORP, total={sum(v1_counts)})")
ax.bar(xs + bw/2, v11_counts, bw, color=C["blue"], alpha=0.85,
        label=f"v1.1 (total={sum(v11_counts)})")
for i, (a, b) in enumerate(zip(v1_counts, v11_counts)):
    if a > 0: ax.text(i - bw/2, a + 5, str(a), ha="center", fontsize=7.5)
    if b > 0: ax.text(i + bw/2, b + 5, str(b), ha="center", fontsize=7.5)
ax.set_xticks(xs); ax.set_xticklabels(faults, fontsize=8)
ax.set_ylabel("# events", fontsize=9)
ax.set_title("(e)  Events by dominant fault", loc="left")
...
save(fig, "FigV18_aggregate_summary",
      plot_data={"hourly_D1": pd.DataFrame({"v1_describe": pd.Series(v1_flat).describe(),
                  ...
                  "fault_counts": pd.DataFrame({"v1": v1_counts, "v11": v11_counts}, index=faults)})
