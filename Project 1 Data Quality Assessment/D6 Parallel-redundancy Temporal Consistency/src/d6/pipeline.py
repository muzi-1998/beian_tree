...
def _fit_and_score(
    ...
    benchmark["D7_screen_pass"] = pd.NA
    benchmark["benchmark_status"] = str(cfg.benchmark["d7_screen_status"])
    benchmark["benchmark_source"] = "v1.2_D1ge4.5_D2continuous24h_D7pending"
    benchmark["inclusion_criteria"] = (
        "D1_target>=4.5; D1_ref>=4.5; bilateral D2 usable for 24 h; "
        "data completeness>=0.80; D7 screen pending"
    )
    ...
    for _, key in group_keys.iterrows():
        ...
        for risk_column, q_column in risk_to_q.items():
            ...
            param_rows.append({
                ...
                "benchmark_source": "v1.2_high_quality_public_D7pending",
    ...
    frame["usable_for_D6"] = data_ok & d2_ok & frame["D6_raw"].notna()
    frame["D6_total"] = frame["D6_raw"].where(frame["usable_for_D6"])

    d1_available = frame["D1_target"].notna() & frame["D1_ref"].notna()
    after_d1, fuse_state = apply_d1_fuse(
        frame["D6_raw"].to_numpy(),
        frame["D1_target"].to_numpy(),
        frame["D1_ref"].to_numpy(),
        frame["usable_for_D6"].to_numpy(),
    ...
    frame["D6_after_D1"] = after_d1
    frame["D6_forDQR_provisional"] = after_d1
    frame["D6_forDQR"] = np.nan
    frame["D6_forDQR_status"] = np.where(
        frame["usable_for_D6"] & d1_available,
        str(cfg.arbitration["provisional_status"]),
        "not_evaluable_or_D1_missing",
    )
    frame["D6_forDQR_is_final"] = False
    frame["D7_zone_consensus_label"] = "not_available"
    frame["D7_zone_consensus_strength"] = np.nan
    ...
    frame["raw_status_label"] = np.select(
        [~frame["usable_for_D6"], frame["D6_raw"].lt(asymmetry_max),
         frame["D6_raw"].ge(consistent_min)],
        ["not_evaluable", "pair_asymmetry", "paired_consistent"],
        default="borderline",
    )
    frame["status_label"] = np.select(
        [~frame["usable_for_D6"], frame["D6_after_D1"].ge(consistent_min),
         frame["fuse_active"]],
        ["not_evaluable", "paired_consistent", "ambiguous_asymmetry"],
        default="pending_D7_arbitration",
    )
    frame["causal_attribution"] = np.where(
        frame["status_label"].eq("pending_D7_arbitration"),
...
def _events(main: pd.DataFrame, min_hours: int) -> pd.DataFrame:
    ...
    for pair_id, frame in main.sort_values("timestamp").groupby("pair_id"):
        active = frame["usable_for_D6"] & frame["D6_raw"].lt(3.0)
        ...
        for _, event in frame[active].groupby(groups[active]):
            ...
            rows.append({
                ...
                "min_D6_forDQR_provisional": event["D6_forDQR_provisional"].min(),
                ...
                "D7_zone_consensus_summary": "pending_D7_not_available",
...
def _profile(main: pd.DataFrame, events: pd.DataFrame) -> pd.DataFrame:
    profile = main.groupby(["pair_id", "zone", "sensor_id", "pair_sensor_id"], as_index=False).agg(
        mean_D6_raw=("D6_total", "mean"), median_D6_raw=("D6_total", "median"),
        p05_D6_raw=("D6_total", lambda x: x.quantile(0.05)),
        mean_D6_provisional=("D6_forDQR_provisional", "mean"),
        evaluable_rate=("usable_for_D6", "mean"), deadband_rate=("deadband_active", "mean"),
    ...
    low_rate = main.groupby("pair_id").apply(
        lambda group: (
            group.loc[group["usable_for_D6"], "D6_raw"].lt(3.0).mean()
            if group["usable_for_D6"].any() else np.nan
        ),
...
def _multiscale(main: pd.DataFrame) -> dict[str, pd.DataFrame]:
    ...
    for label, freq in (("daily", "1D"), ("weekly", "W-MON")):
        ...
        for pair_id, frame in source.groupby("pair_id"):
            raw = frame["D6_total"].resample(freq).agg(
                D6_raw_gate=lambda x: x.quantile(0.05),
                D6_raw_report=lambda x: x.quantile(0.25),
                D6_raw_mean="mean", D6_raw_median="median", n_windows="count",
            )
            provisional = frame["D6_forDQR_provisional"].resample(freq).agg(
                D6_provisional_gate=lambda x: x.quantile(0.05),
                D6_provisional_report=lambda x: x.quantile(0.25),
                D6_provisional_mean="mean",
            )
            agg = raw.join(provisional)
            agg["pair_id"] = pair_id
            agg["D6_forDQR_status"] = "pending_D7_arbitration"
...
def run_pipeline(project_root: Path, d6_root: Path) -> dict[str, object]:
    ...
    run_id = datetime.now(timezone.utc).strftime("D6V14_%Y%m%d_%H%M%S")
    ...
    calibration_id = f"D6CAL-V14-{calibration_digest[:12]}"
    main["run_id"] = run_id
    ...
    score_columns = [
        ...
        "D6_after_D1", "D6_forDQR_provisional", "D6_forDQR", "D6_forDQR_status",
        "D6_forDQR_is_final", "raw_status_label", "status_label", "causal_attribution",
        "fuse_state", "fuse_active", "dominant_evidence", "second_evidence",
        "deadband_active", "deadband_used", "D1_target", "D1_ref",
        "D7_zone_consensus_label", "D7_zone_consensus_strength",
        "D2_target", "D2_ref", "D2_target_tag", "D2_ref_tag", "D2_target_veto",
        "D2_ref_veto", "D2_target_continuous_24h", "D2_ref_continuous_24h",
        "valid_fraction_target", "valid_fraction_reference", "usable_for_D6",
    ...
    _write_excel(output_dir / "D6_mapping_params.xlsx", {
        ...
        "version": pd.DataFrame([{
            ...
            "D7_status": "pending_not_available",
        }]),
    })
    _write_excel(output_dir / "D6_pair_benchmark_library.xlsx", {
        "benchmark_windows": benchmark[[
            ...
            "D2_ref_continuous_24h", "D7_screen_pass", "benchmark_status",
    ...
    boundary = pd.DataFrame([
        {"layer": "D2 continuity", "effect": "gate D6 evaluability", "output": "usable_for_D6"},
        {"layer": "D1 bilateral fuse", "effect": "true v1.2 fuse", "output": "D6_after_D1"},
        {"layer": "D7 zone consensus", "effect": "not available; no proxy generated", "output": "pending_D7_arbitration"},
    ])
    arbitration = main[[
        "timestamp", "pair_id", "D6_raw", "D1_target", "D1_ref", "fuse_state",
        "D6_after_D1", "D7_zone_consensus_label", "D6_forDQR_provisional",
        "D6_forDQR", "D6_forDQR_status", "D6_forDQR_is_final",
    ]]
    _write_excel(output_dir / "D6_arbitration_log.xlsx", {
    ...
    for name, path in (
        ("d6_config", config_path),
        ("d6_pipeline_code", Path(__file__)),
        ("d6_scoring_code", Path(__file__).with_name("scoring.py")),
    ):
        dependencies.append({
            "dependency": name, "path_role": path.name, "sha256": _sha256(path),
            "modified_utc": datetime.fromtimestamp(path.stat().st_mtime, timezone.utc).isoformat(),
    ...
    audit = pd.DataFrame([
        ...
        {"key": "evaluable_rate_D6_raw", "value": float(main["usable_for_D6"].mean())},
        {"key": "D6_forDQR_status", "value": "provisional only; pending D7 arbitration"},
        {"key": "benchmark_D7_screen", "value": "pending; no D7 proxy generated"},
        {"key": "causal_claim", "value": "pair asymmetry only; sensor/process cause pending D7"},
    ])
    ...
    manifest = {
        ...
        "scientific_boundary": (
            "D6_raw is independent; D2 gates evaluability; true D1 fuse produces a provisional "
            "downstream score; final D6_forDQR is unavailable until real D7 arbitration."
        ),
        "benchmark_status": "provisional_pending_D7_screen",
    }
