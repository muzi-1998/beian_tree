...
def _load_score(path: Path, version: str, evaluable_column: str) -> pd.DataFrame:
    ...
    return frame[["timestamp", "pair_id", "D6_raw", "evaluable", "version"]]
...
def _standardized_event_counts(frame: pd.DataFrame, min_hours: int = 3) -> pd.Series:
    ...
    for pair_id, pair in frame.sort_values("timestamp").groupby("pair_id"):
        active = pair["evaluable"] & pair["D6_raw"].lt(3.0)
...
def _summary(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    for (version, pair_id), group in frame.groupby(["version", "pair_id"], sort=False):
        valid = group.loc[group["evaluable"], "D6_raw"].dropna()
        rows.append({
            "version": version,
            "pair_id": pair_id,
            "n_rows": len(group),
            ...
            "mean_D6_raw": valid.mean(),
            "median_D6_raw": valid.median(),
            "p05_D6_raw": valid.quantile(0.05),
            "p25_D6_raw": valid.quantile(0.25),
            "p75_D6_raw": valid.quantile(0.75),
            "p95_D6_raw": valid.quantile(0.95),
...
def _aligned_comparisons(frame: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    wide = frame.pivot_table(
        index=["timestamp", "pair_id"], columns="version", values="D6_raw", aggfunc="first"
    ).reset_index()
    rows = []
...
def _comparison_figure(
    ...
    for offset, version, color in zip((-width, 0.0, width), VERSION_ORDER, VERSION_COLORS):
        ...
        axes[0, 0].bar(x + offset, values["mean_D6_raw"], width=width, color=color, label=version)
    ...
    axes[1, 0].set(
        xlim=(1, 5), ylim=(1, 5), xlabel="Current v1.3 D6 raw", ylabel="Restored v1.4 D6 raw"
    )
    for letter, ax in zip("abcd", axes.ravel()):
        panel_label(ax, letter)
    save_figure(fig, output_dir / "Fig_D6_three_version_sensitivity")


def run_sensitivity(d6_root: Path) -> dict[str, pd.DataFrame]:
    sources = (
        ("Legacy v1.2 proxy", d6_root / "legacy" / "2026-05-30-proxy" / "D6_main_scores.xlsx", "usable_for_DQR"),
        ("Current v1.3", d6_root / "legacy" / "2026-07-13-v1.3-independent" / "D6_main_scores.xlsx", "usable_for_DQR"),
        ("Restored v1.4", d6_root / "outputs" / "data" / "D6_main_scores.xlsx", "usable_for_D6"),
    )
    combined = pd.concat(
    ...
    pair_wide = summary.pivot(index="pair_id", columns="version", values="mean_D6_raw").reset_index()
    ...
    method_changes = pd.DataFrame([
        {"version": "Legacy v1.2 proxy", "input": "raw minute data; internal residual", "mapping": "pooled by regime", "change_point": "legacy adjacent-KS implementation", "arbitration": "D1/D2/D7 proxies", "event_rule": "3 h"},
        ...
        {"version": "Restored v1.4", "input": "formal 1.1 residual; 10-min median", "mapping": "public variable x regime; documented fallback", "change_point": "7-d adjacent KS; fixed timing table", "arbitration": "real D1 fuse; D7 pending; no proxy", "event_rule": "3 h"},
    ])
    output_dir = d6_root / "outputs" / "comparison"
    output_dir.mkdir(parents=True, exist_ok=True)
    output_path = output_dir / "D6_three_version_sensitivity.xlsx"
