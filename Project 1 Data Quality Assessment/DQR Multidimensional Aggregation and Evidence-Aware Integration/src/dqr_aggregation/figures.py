...
EXT_STEMS = ["DQR_ExtFig01_pair_weighting_sensitivity"]
ALL_STEMS = STEMS + EXT_STEMS
...
def _figure_2(
    ...
    fig, axes = _new_figure(168, columns=2, rows=3)
    ...
    ax = axes[0, 1]
    panel_label(ax, "b")
    ax.set_title("D5 support migration", loc="left")
    support_bottom = np.zeros(len(all_nodes))
    for column, label, color in (
        ("D5_L1_rate", "L1", PALETTE["light_gray"]),
        ("D5_L2_rate", "L2", PALETTE["gold"]),
        ("D5_L3_rate", "L3", PALETTE["blue"]),
    ):
        values = all_nodes[column].to_numpy(float)
        ax.bar(x, values, bottom=support_bottom, width=0.72, color=color, label=label)
        support_bottom += values
    no_support = np.clip(1.0 - support_bottom, 0.0, 1.0)
    ax.bar(
        x,
        no_support,
        ...
        width=0.72,
        color="#F1F2F3",
        label="No reportable support",
    )
    ax.plot(
        x,
        all_nodes["D5_ood_rate"],
        marker="o",
        ms=2.6,
        color=PALETTE["red"],
        label="OOD",
    ...
    ax = axes[2, 0]
    panel_label(ax, "e")
    ax.set_title("Selection and composition decomposition", loc="left")
    positions = np.arange(len(effect_order))
    estimates = overall["estimate"].to_numpy(float)
    errors = np.vstack(
        [
            estimates - overall["ci_low"].to_numpy(float),
            overall["ci_high"].to_numpy(float) - estimates,
        ]
    )
    ax.errorbar(
        positions,
    ...
    ax.axhline(0, color=PALETTE["gray"], lw=0.7, ls=":")
    ax.set_xticks(positions, effect_labels)
    ax.set_ylabel("Score difference (95% block CI)")
    style_axes(ax)

    analyte = decomposition.loc[decomposition["stratum_type"].eq("analyte")].copy()
    ax = axes[2, 1]
    panel_label(ax, "f")
    ax.set_title("Decomposition by analyte", loc="left")
    for offset, (name, color) in zip((-0.11, 0.11), (("DO", PALETTE["blue"]), ("ORP", PALETTE["orange"]))):
        frame = analyte.loc[analyte["stratum"].eq(name)].set_index("effect").loc[effect_order]
        estimate = frame["estimate"].to_numpy(float)
        error = np.vstack(
            [
                estimate - frame["ci_low"].to_numpy(float),
                frame["ci_high"].to_numpy(float) - estimate,
            ]
        )
        ax.errorbar(
            positions + offset,
    ...
    write_workbook(
        ...
        {
            "monthly_coverage": all_nodes,
            "monthly_quality_evidence": quality,
...
def _case_definitions(config: dict[str, Any], node: pd.DataFrame, pair: pd.DataFrame) -> list[dict[str, Any]]:
    ...
    settings = config["case_studies"]
    cases: list[dict[str, Any]] = []
    sensor = str(settings["stable_reference_sensor"])
    reference_end = pd.Timestamp(config["study"]["start"]) + (
        pd.Timestamp(config["study"]["end"]) - pd.Timestamp(config["study"]["start"])
    ) * float(settings["stable_reference_fraction"])
    stable = node.loc[node["sensor_id"].eq(sensor)].sort_values("timestamp").copy()
    stable["eligible"] = (
        stable["coverage_class"].eq("full")
        & stable["D3_gate_status"].eq("Pass")
        & stable["timestamp"].le(reference_end)
    )
    window = int(settings["stable_window_hours"])
    stable["stable_window"] = (
        stable["eligible"].astype(int).rolling(window, center=True, min_periods=window).sum().eq(window)
    )
    candidates = stable.loc[stable["stable_window"] & stable["Q_node_full"].notna()].copy()
    if candidates.empty:
        raise RuntimeError("No pre-specified stable complete-evidence reference window")
    target = float(candidates["Q_node_full"].median())
    normal = candidates.loc[(candidates["Q_node_full"] - target).abs().idxmin()]
    cases.append(
        {
            "case_id": "benchmark_median_stable_window",
            "sensor_id": normal["sensor_id"],
            "center_timestamp": normal["timestamp"],
            "case_type": "Benchmark stable window",
            "truth_status": "prespecified_rule_not_field_truth",
            "selection_rule": "median_Q_within_73h_stable_complete_evidence_development_window",
        }
    )
    selected_ids = list(settings["D5_case_ids"])
    selected = index.set_index("case_id").loc[selected_ids].reset_index()
    for _, row in selected.iterrows():
        record = row.to_dict()
        record["case_type"] = "Pre-specified D5 review window"
        record["selection_rule"] = "predefined_D5_case_registry"
        cases.append(record)
    d4_path = PROJECT_ROOT / config["inputs"]["D4"]["formal_case_path"]
    registry = pd.read_excel(d4_path, sheet_name="episode_registry")
    event = registry.loc[registry["event_id"].eq(settings["D4_formal_event_id"])]
    if len(event) != 1:
        raise RuntimeError("The pre-specified D4 formal event is missing or non-unique")
    event = event.iloc[0]
    start = pd.Timestamp(event["start_ts"])
    end = pd.Timestamp(event["end_ts"])
    cases.append(
        {
            "case_id": event["event_id"],
            "sensor_id": settings["D4_display_sensor"],
            "center_timestamp": start + (end - start) / 2,
            "case_type": "Registered D4 episode",
            "truth_status": event["causal_attribution"],
            "selection_rule": "predefined_D4_formal_episode_registry",
        }
    )
    return cases[:4]
...
def _figure_5(
    ...
    for index, (ax, case) in enumerate(zip(axes.ravel(), cases)):
        ...
        case_title = str(case["case_type"]).replace("_", " ").strip()
        ax.set_title(
...
def _extended_pair_weighting_figure(
    figure_root: Path,
    source_root: Path,
    ...
    hb = ax.hexbin(
        rows["Q_pair_hierarchical"],
        rows["Q_pair_native_atom_equal"],
        gridsize=38,
        mincnt=1,
        bins="log",
        cmap="Blues",
    ...
    ax.plot([1, 5], [1, 5], color=PALETTE["gray"], lw=0.8, ls=":")
    ax.axvline(3, color=PALETTE["red"], lw=0.6, ls="--")
    ax.axhline(3, color=PALETTE["red"], lw=0.6, ls="--")
    ax.set(xlim=(1, 5.02), ylim=(1, 5.02))
    ax.set_xlabel("Hierarchical equal-component score")
    ax.set_ylabel("Native-atom equal score")
    style_axes(ax)
    cbar = fig.colorbar(hb, ax=ax, pad=0.02, fraction=0.05)
    cbar.set_label("Pair-hours (log scale)")
    cbar.outline.set_linewidth(0.6)

    metrics = summary.iloc[0]
    ax = axes[0, 1]
    panel_label(ax, "b")
    ax.set_title("Low-tail sensitivity", loc="left")
    positions = np.arange(2)
    width = 0.34
    hierarchical = [
        metrics["hierarchical_low_hours_per_1000"],
        metrics["hierarchical_events_per_1000"],
    ]
    native = [
        metrics["native_atom_low_hours_per_1000"],
        metrics["native_atom_events_per_1000"],
    ]
    ax.bar(positions - width / 2, hierarchical, width, color=PALETTE["navy"], label="Hierarchical")
    ax.bar(positions + width / 2, native, width, color=PALETTE["gold"], label="Native atom")
    ax.set_xticks(positions, ["Low-score hours", "Episodes"])
    ax.set_ylabel("Burden per 1,000 pair-hours")
    ax.legend(loc="upper right")
    ax.text(
        0.02,
        0.97,
        f"Spearman $\\rho$ = {metrics['spearman']:.3f}\n"
        f"Low-tail Jaccard = {metrics['low_tail_jaccard']:.3f}\n"
        f"Decision flips = {100 * metrics['decision_flip_rate_at_3']:.2f}%",
        transform=ax.transAxes,
        va="top",
        ha="left",
        fontsize=6.2,
...
def build_all_figures(
    ...
    *,
    ...
    paths.extend(_figure_2(figure_root, source_root, node, monthly, decomposition))
