...
CONFIG_PATH = DQR_ROOT / "configs" / "aggregation_v2_4.yaml"
OUTPUT_ROOT = DQR_ROOT / "outputs" / "aggregation_v2_4"


def load_config() -> dict[str, Any]:
    payload = yaml.safe_load(CONFIG_PATH.read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise ValueError(f"{CONFIG_PATH.name} must contain a mapping")
    return payload
...
def make_run_id(config: dict[str, Any]) -> str:
    ...
    return f"DQRAGG-V24-{hashlib.sha256(payload).hexdigest()[:12]}"
