...
def build_events(
    ...
    *,
    ...
    for sensor, frame in scores.sort_values("timestamp").groupby("sensor_id"):
        ...
        active = frame["D7_raw"].lt(low_score_threshold) & frame["D7_raw"].notna()
        ...
        for _, event_frame in frame[active].groupby(group[active]):
            ...
            rows.append(
                {
                    "event_id": f"D7-EVT-{start:%Y%m}-{event_no:04d}",
                    ...
                    "affected_edges": "[]",
                    "min_D7_raw": float(event_frame["D7_raw"].min()),
                    "mean_D7_raw": float(event_frame["D7_raw"].mean()),
                    "min_D7_total": float(event_frame["D7_total"].min()) if event_frame["D7_total"].notna().any() else np.nan,
                    "mean_D7_total": float(event_frame["D7_total"].mean()) if event_frame["D7_total"].notna().any() else np.nan,
