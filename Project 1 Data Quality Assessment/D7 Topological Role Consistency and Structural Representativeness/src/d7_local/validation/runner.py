...
from d7_local.outputs import D7OutputExporter
from d7_local.templates import SpatialTemplate
...
class D7ValidationRunner:
    def __init__(self, n_trials_per_scenario: int = 60, random_seed: int = 42) -> None:
        ...
        self.topology = TopologyRegistry.load(D7_ROOT / "configs" / "common")
        self.windows = load_yaml(D7_ROOT / "configs" / "common" / "windows.yaml")
    ...
    def run(self) -> dict[str, Any]:
        ...
        state = pd.read_parquet(self.paths.local_output_root / "D7_regime_state.parquet")
        ...
        for scenario in scenarios:
            ...
            for trial_no, (target, start) in enumerate(schedule):
                ...
                trial_rows.append(
                    {
                        "trial_id": f"D7-INJ-{scenario}-{trial_no + 1:03d}",
                        "scenario": scenario,
                        "intensity": severity,
                        "duration_h": 24,
        ...
        self.exporter.write_workbook("D7_ablation_redundancy", ablation)
        return {
    ...
    def _load_templates(self) -> dict[tuple[str, int], SpatialTemplate]:
        path = self.paths.local_output_root / "D7_spatial_templates.template_bundle.json"
    ...
    def _window_stat(
        ...
        d7_raw = float(self.aggregation["lambda_blend"]) * base + (
            1.0 - float(self.aggregation["lambda_blend"])
        ) * minimum
        d7_anomaly = (5.0 - d7_raw) / 4.0
        ...
        return d7_anomaly, node_influence
    ...
    def _negative_controls(
        ...
        for scenario in scenarios:
            for trial_no in range(self.n_trials):
                ...
                rows.append(
                    {
                        "trial_id": f"D7-NEG-{scenario}-{trial_no + 1:03d}",
                        "scenario": scenario,
                        "start_ts": start,
                        "duration_h": 24,
    ...
    def _ablation(self) -> dict[str, pd.DataFrame]:
        main = pd.read_parquet(self.paths.local_output_root / "D7_main_scores_hourly.parquet")
        ...
        invariance_path = self.paths.sensitivity_output_root / "D7_track_invariance.xlsx"
        track = pd.read_excel(invariance_path, sheet_name="track_invariance")
        shadow = pd.read_parquet(self.paths.sensitivity_output_root / "D7_shadow_scores.parquet")
        joined = main[["timestamp", "sensor_id", "D7_raw"]].merge(
            shadow[["timestamp", "sensor_id", "D1_score", "D2_score", "D4_score"]],
            on=["timestamp", "sensor_id"],
            how="inner",
        )
        correlations = []
        for dimension in ["D1_score", "D2_score", "D4_score"]:
            correlations.append(
                {
                    "dimension": dimension,
                    "spearman_with_D7": spearmanr(
                        joined["D7_raw"], joined[dimension], nan_policy="omit"
                    ).statistic,
