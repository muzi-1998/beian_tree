...
def build_report_interface(main: pd.DataFrame) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"D7 report interface is missing columns: {missing}")
    output = main[REPORT_COLUMNS].copy()
    if output.duplicated(["timestamp", "sensor_id"]).any():
        raise ValueError("D7 report interface contains duplicate sensor-hours")
    return output
...
def build_gate_interface(consensus: pd.DataFrame) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"D7 gate interface is missing columns: {missing}")
    output = frame[GATE_COLUMNS].copy()
    if output.duplicated(["timestamp", "pair_id"]).any():
        raise ValueError("D7 gate interface contains duplicate pair-hours")
    if not output["veto_active"].equals(
        output["sensor_identity_veto_active"]
    ):
        raise ValueError(
            "Only sensor-identity evidence may activate D7 Veto"
        )
    return output
