...
def build_manifest(
    ...
    *,
    ...
    artifacts = []
    for path in sorted(output_root.glob("D7_*")):
        if path.name == "D7_run_manifest.json" or not path.is_file():
            continue
        artifacts.append(
            {
                "path": path.name,
    ...
    manifest = {
        **identity,
        ...
        "track_isolation": {
            ...
            "consumed_sources": [],
            "forbidden_score_dimensions": ["D1", "D2", "D4", "D5", "D6"],
        },
        "artifacts": artifacts,
        "scientific_boundaries": scientific_boundaries,
        "acceptance": acceptance,
    }
    target = output_root / "D7_run_manifest.json"
...
def _role(name: str) -> str:
    ...
    if "zone_consensus" in name or "d6_interface" in name:
        return "d6_read_only_interface"
    if "topology" in name:
